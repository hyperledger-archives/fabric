...
func NewChaincodeHandler(to string, peerChatStream chaincode.PeerChaincodeStream, chaincode Chaincode, stub ChaincodeStub) *Handler {
	...
	v.FSM = fsm.NewFSM(
		...
		fsm.Events{
			...
			{Name: pb.ChaincodeMessage.INIT.String(), Src: []string{"established"}, Dst: "init"},
			{Name: pb.ChaincodeMessage.ERROR.String(), Src: []string{"init"}, Dst: "established"},
			{Name: pb.ChaincodeMessage.RESPONSE.String(), Src: []string{"init"}, Dst: "init"},
			{Name: pb.ChaincodeMessage.COMPLETED.String(), Src: []string{"init"}, Dst: "ready"},
			{Name: pb.ChaincodeMessage.TRANSACTION.String(), Src: []string{"ready"}, Dst: "transaction"},
			{Name: pb.ChaincodeMessage.COMPLETED.String(), Src: []string{"transaction"}, Dst: "ready"},
			{Name: pb.ChaincodeMessage.ERROR.String(), Src: []string{"transaction"}, Dst: "ready"},
			{Name: pb.ChaincodeMessage.RESPONSE.String(), Src: []string{"transaction"}, Dst: "transaction"},
		},
		fsm.Callbacks{
...
func (handler *Handler) BeforeInit(e *fsm.Event) {
	...
	go func() {
		...
		input := msg.Payload.(*pb.ChaincodeInput)	
...
func (handler *Handler) BeforeTransaction(e *fsmEvent) {
	msg, ok := e.Args[0].(*pb.ChaincodeMessage)
	if !ok {
		e.Cancel(fmt.Errorf("Received unexpected message type"))
		return
...
func (handler *Handler) BeforeResponse(e *fsmEvent) {
	msg, ok := e.Args[0].(*pb.ChaincodeMessage)
	if !ok {
		e.Cancel(fmt.Errorf("Received unexpected message type"))
		return
