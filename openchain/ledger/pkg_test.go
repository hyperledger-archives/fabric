...
func (testWrapper *blockchainTestWrapper) populateBlockChainWithSampleData() (blocks []*protos.Block, hashes [][]byte) {
	...
	// -----------------------------<Block 2>-------------------------------------
	// Deploy a chaincode
	transaction2a := protos.NewTransaction(protos.ChaincodeID{Path: &protos.ChaincodePath{Url: "Contracts"}}, testutil.GenerateUUID(testWrapper.t), "NewContract", []string{"name: MyContract1, code: var x; function setX(json) {x = json.x}}"})
	...
	transaction3a := protos.NewTransaction(protos.ChaincodeID{Path: &protos.ChaincodePath{Url: "MyContract"}}, testutil.GenerateUUID(testWrapper.t), "setX", []string{"{x: \"hello\"}"})
...
func buildTestTx() (*protos.Transaction, string) {
	...
	return protos.NewTransaction(protos.ChaincodeID{Path: &protos.ChaincodePath{Url: "testUrl", Version: "1.1"}}, uuid, "anyfunction", []string{"param1, param2"}), uuid
}
