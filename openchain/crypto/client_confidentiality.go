...
func (client *clientImpl) encryptTx(tx *obc.Transaction) error {
	...
	client.node.log.Debug("confidentiality protocol version [%s]", tx.ConfidentialityProtocolVersion)
	switch tx.ConfidentialityProtocolVersion {
	case "1.1":
		client.node.log.Debug("Using confidentiality protocol version 1.1")
		return client.encryptTxVersion1_1(tx)
	case "1.2":
		client.node.log.Debug("Using confidentiality protocol version 1.2")
