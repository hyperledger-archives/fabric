...
func TestCLI(t *testing.T) {
	...
	privateKey, err := cli.NewPrivateKey(nil, ecdsaKey)
	if err != nil {
		t.Fatal(err)
	}
	...
	rawKey, err := cli.SerializePrivateKey(privateKey)
	if err != nil {
		t.Fatal(err)
	}

	privateKey, err = cli.DeserializePrivateKey(rawKey)
	if err != nil {
		t.Fatal(err)
	}

	// Encrypt
	cipher, err := cli.NewAsymmetricCipherFromPublicKey(privateKey.GetPublicKey())
	if err != nil {
		t.Fatal(err)
	}
	...
	// Decrypt
	cipher, err = cli.NewAsymmetricCipherFromPrivateKey(privateKey)
	if err != nil {
		t.Fatal(err)
	}
