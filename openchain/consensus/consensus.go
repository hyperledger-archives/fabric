...
import (
	...
	"github.com/openblockchain/obc-peer/openchain/consensus/plugin"
...
// `r.parent` and do a `r.stream.SendMessage(msg)` on each one of them. (Note:
// This is a TODO). For now, there's only one `streamReceiver` per
// `consensusLayer`.
...
func (layer *consensusLayer) ExecTXs(ctxt context.Context, txs []*pb.Transaction) ([]byte, []error) {
	...
	for i, t := range txs {
		...
		_, errors[i] = VMCProcess(ctxt, DOCKER, req)
	}
...
func (layer *consensusLayer) GetReceiver(stream openchain.PeerChatStream) messageHandler {
	...
	// If we haven't established a receiver for that stream, initialize one.
	if layer.receiver == (streamReceiver{}) {
		if Logger.IsEnabledFor(logging.DEBUG) {
