...
WORKDIR $GOPATH/src/github.com/hyperledger-incubator/obc-peer/
COPY . .
RUN CGO_CFLAGS="-I/opt/rocksdb/include" CGO_LDFLAGS="-L/opt/rocksdb -lrocksdb -lstdc++ -lm -lz -lbz2 -lsnappy" go install && cp $GOPATH/src/github.com/hyperledger-incubator/obc-peer/openchain.yaml $GOPATH/bin
RUN cp $GOPATH/src/github.com/hyperledger-incubator/obc-peer/openchain/consensus/obcpbft/config.yaml $GOPATH/bin
