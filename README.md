...
The `node start` command will start the peer process. You can then use the other commands to interact with the peer process. For example, the `node status` command will return the status of the running peer. The full list of commands is the following:
