...
function hkdf(ikm, keyBitLength, salt, info, Hash) {
    ...
    hmac.update(ikm);
    key = hmac.digest();
    // console.log("prk: %j", new Buffer(bitsToBytes(key)).toString('hex'));
    ...
    for (i = 1; i <= loops; i++) {
        ...
        hmac.update(curOut);
        hmac.update(info);
        // console.log('sjcl.bitArray.partial(8, i): %j', sjcl.bitArray.partial(8, i));
