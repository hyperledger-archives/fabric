...
func (d *Devops) Deploy(ctx context.Context, spec *pb.ChaincodeSpec, attributes map[string]string ) (*pb.ChaincodeDeploymentSpec, error) {
	...
	if viper.GetBool("security.enabled") {
		...
		tx, err = sec.NewChaincodeDeployTransaction(chaincodeDeploymentSpec, attributes, transID)
		if nil != err {
			return nil, err
		}
	} else {
...
func (d *Devops) Invoke(ctx context.Context, chaincodeInvocationSpec *pb.ChaincodeInvocationSpec,  attributes map[string]string ) (*pb.Response, error) {
	return d.invokeOrQuery(ctx, chaincodeInvocationSpec, attributes,  true)
}

// Query performs the supplied query on the specified chaincode through a transaction
func (d *Devops) Query(ctx context.Context, chaincodeInvocationSpec *pb.ChaincodeInvocationSpec, attributes map[string]string ) (*pb.Response, error) {
	return d.invokeOrQuery(ctx, chaincodeInvocationSpec,  attributes, false)
}
